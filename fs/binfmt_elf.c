		...
		unsigned long total_size)
{
	...
	*/
	if (total_size) {
        ...
        masked_map_addr = gem5_aslr_remove_rand_offset(map_addr);
		if (!BAD_ADDR(masked_map_addr))
	...
	if ((type & MAP_FIXED_NOREPLACE) &&
	    PTR_ERR((void *)masked_map_addr) == -EEXIST)
...
//                pr_info("@@@ load_elf_binary_counter %lx", load_elf_binary_counter);
				...
				if (current->flags & PF_RANDOMIZE) {
                    ...
                    load_bias += get_gem5_user_high_offset();
//                    load_bias += strcmp(bprm->file->f_path.dentry->d_name.name, "kmod") == 0 ? 0x10000000000 : 0; // 0x1010000000000 : 0; // 0x1fd // 0x1000000000000; // arch_mmap_rnd();
#endif
                }
...
//            pr_info("@@@ Interp load_addr %lx entry %lx\n", interp_load_addr, elf_entry);
        ...
        unsigned long masked_elf_entry = gem5_aslr_remove_rand_offset(elf_entry);
		if (BAD_ADDR(masked_elf_entry)) {
			retval = IS_ERR_VALUE(masked_elf_entry) ?
					(int)masked_elf_entry : -EINVAL;
			goto out_free_dentry;
		}
